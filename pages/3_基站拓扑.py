...
base_coords, real_hourly_loads = load_real_nodes_and_loads()
    ...
    "baseOption": {
        ...
        "series": [
            ...
            # 基站数据
            {
                ...
                "symbolSize": 12,
                "label": {"show": True, "formatter": JsCode("function(params) { return params.data[4]; }"), "position": "right", "color": "#c0c0c0", "fontSize": 12},
                "itemStyle": {"shadowBlur": 10, "shadowColor": "rgba(0, 255, 136, 0.3)"},
                "emphasis": {"itemStyle": {"shadowBlur": 20, "shadowColor": "rgba(0, 255, 136, 0.6)"}},
                "animation": True,
...
# 第二行：实时统计卡片（纵向）+ 24小时趋势图
stats_col, trend_col2 = st.columns([1, 2])
...
with stats_col:
    ...
    <div style="background: linear-gradient(135deg, rgba(0, 170, 255, 0.2) 0%, rgba(0, 170, 255, 0.1) 100%);
                ...
                cursor: pointer;"
         title="当前显示的时段">
        <div style="color: #00aaff; font-size: 16px; font-weight: bold; margin-bottom: 3px;">当前时段</div>
...
with trend_col2:
    ...
    st_echarts(options=trend_option, height="320px", key="hourly_trend_chart")

st.markdown("---")

# 创建两列布局：左侧基站负载热力图，右侧3D地球
heatmap_col, globe_col = st.columns([1, 1])

with heatmap_col:
    st.markdown("##### 基站负载热力图")
    
    # 使用真实数据生成热力图数据（只使用当前时段的数据，避免重复）
    heatmap_data = []
    current_hour_code = 12 // 6  # 使用12点的数据作为示例
    for node in base_coords:
        x = node["x"]
        y = node["y"]
        node_id = node["node_id"]
        ...
        # 使用真实负载数据
        if current_hour_code in real_hourly_loads and node_id in real_hourly_loads[current_hour_code]:
            load = real_hourly_loads[current_hour_code][node_id]
        else:
            # 后备模拟数据
            load = np.random.uniform(20, 80)
        
        # 确保数据有效
        if load is not None and not (isinstance(load, float) and np.isnan(load)):
            heatmap_data.append([float(x), float(y), round(float(load), 1)])

    # 使用散点图代替热力图，避免插值形成大片颜色
    scatter_option = {
        "backgroundColor": "transparent",
        "tooltip": {
            "trigger": "item",
            "formatter": JsCode("function(params) { return '邮政编码: ' + params.data[3] + '<br/>经度: ' + params.data[0].toFixed(4) + '<br/>纬度: ' + params.data[1].toFixed(4) + '<br/>负载: ' + params.data[2].toFixed(1) + '%'; }")
        },
        ...
        "visualMap": {
            ...
            "inRange": {"color": ["#50a3ba", "#eac736", "#d94e5d"]},
            "textStyle": {"color": "#c0c0c0", "fontSize": 10}
        },
        "series": [{
            "type": "scatter",
            "data": [[d[0], d[1], d[2], node['postal_code']] for d, node in zip(heatmap_data, base_coords)],
            "symbolSize": 12,
            "label": {"show": False},
            "itemStyle": {
                "borderWidth": 1,
    ...
    st_echarts(options=scatter_option, height="400px", key="scatter_chart")
